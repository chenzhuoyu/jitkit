    ...
    switch p.Op {
        ...
        case hir.OP_sp: {
            self.Ins = append(
                ...
                &IrConstPtr {
                    R: Pr(1),
                    P: rt.V_pWriteBarrier,
                    M: Volatile,
                },
                &IrConstPtr {
                    R: Pr(2),
                    P: rt.F_gcWriteBarrier,
                },
                &IrWriteBarrier {
                    R   : Rv(p.Ps),
                    M   : Pr(0),
        ...
        case hir.OP_bzero: {
            ...
            for ; n >= rt.MaxZeroSize; n -= rt.MaxZeroSize {
                self.zeroBlock(r, d, rt.MaxZeroSize)
                d += rt.MaxZeroSize
            }
...
func (self *BasicBlock) termReturn(p *hir.Ir) {
    self.Term = &IrReturn { R: ri2regs(p.Rr[:p.Rn]) }
}
